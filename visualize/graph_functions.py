...
def add_sheet_nodes_to_graph(G,datastore,n,sheet):
	...
	nodes_with_sheet = [(i+n,
							{"sheet":sheet,"coor":(positions['x'][i],positions['y'][i])})
						for i
						in node_indices]
...
def sheet_graph_to_show(G,sheet,selection=None,edges_in=False):
	if selection==None: # works (shows also edges)
