...
def test_get_ads_list(datastore: mocks.DataStoreMock):
    datastore.analysis_results = [
        mocks.DSAdsMock(analysis_algorithm="alg5",
                        identifier=AdsIdentifier.PerNeuronValue.value, tags=['t1', 't2']),
        mocks.DSAdsMock(analysis_algorithm="alg3",
                        identifier=AdsIdentifier.SingleValue.value, tags=['t1', 't2']),
        mocks.DSAdsMock(analysis_algorithm="alg1",
                        identifier=AdsIdentifier.Connections.value),
        mocks.DSAdsMock(analysis_algorithm="alg5",
                        identifier=AdsIdentifier.PerNeuronValue.value, tags=['t2', 't1']),
        mocks.DSAdsMock(analysis_algorithm="alg4",
                        identifier=AdsIdentifier.PerNeuronValue.value, tags=['t2', 't1']),
        mocks.DSAdsMock(analysis_algorithm="alg2",
                        identifier=AdsIdentifier.ConductanceSignalList.value),
        mocks.DSAdsMock(analysis_algorithm="alg6",
                        identifier=AdsIdentifier.Connections.value),
        mocks.DSAdsMock(analysis_algorithm="alg7",
                        identifier="foo", tags=['t1']),
    ]
    assert get_ads_list('path') == [
        Ads(algorithm="alg2",
            identifier=AdsIdentifier.ConductanceSignalList.value, tags=[]),
        Ads(algorithm="alg3", identifier=AdsIdentifier.SingleValue.value,
            tags=['t1', 't2']),
        Ads(algorithm="alg4",
            identifier=AdsIdentifier.PerNeuronValue.value, tags=['t1', 't2']),
        Ads(algorithm="alg5",
            identifier=AdsIdentifier.PerNeuronValue.value, tags=['t1', 't2']),
        Ads(algorithm="alg7", identifier="foo", tags=['t1']),
...
def test_get_per_neuron_value(datastore: mocks.DataStoreMock):
    datastore.get_analysis_result.return_value = [
        ...
        mocks.DSPnvMock(
            ...
            ids=[1, 2, 3, 4],
            period=None,
            value_units=mocks.QuantitiesMock(
